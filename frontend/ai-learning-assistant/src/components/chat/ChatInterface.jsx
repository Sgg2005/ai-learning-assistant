...
const ChatInterface = () => {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      if (!activeSessionId) {
        ...
        setSessions((prev) => prev.map((s) => (s._id === sessionId ? { ...s, sessionName } : s)));
      }
    } catch (error) {
      setHistory((prev) => [
        ...prev,
  ...
  const handleRename = async (sessionId) => {
    ...
    try {
      ...
      setSessions((prev) =>
        prev.map((s) => (s._id === sessionId ? { ...s, sessionName: renameValue.trim() } : s))
      );
      setRenamingId(null);
    } catch {
        ...
        <div
          className={`max-w-2xl p-4 rounded-2xl shadow-sm ${
            isUser
              ...
              : 'bg-white dark:bg-slate-800/90 border border-slate-200 dark:border-slate-600 text-slate-800 dark:text-slate-100 rounded-tl-md'
          }`}
        >
          {isUser ? (
            <p className="text-sm leading-7">{msg.content}</p>
          ) : (
            <div className="text-sm leading-7 text-slate-800 dark:text-slate-100 [&_p]:text-slate-800 dark:[&_p]:text-slate-100 [&_li]:text-slate-700 dark:[&_li]:text-slate-200 [&_strong]:text-slate-900 dark:[&_strong]:text-white">
  ...
  if (initialLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-16 gap-4">
        ...
        <Spinner />
        <p className="text-slate-500 dark:text-slate-400 text-sm">Loading chats...</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col md:flex-row gap-4 h-auto md:h-[75vh]">
      <div className="w-full md:w-60 max-h-48 md:max-h-none shrink-0 bg-white dark:bg-slate-800 border border-orange-100 dark:border-slate-700 rounded-2xl shadow-sm flex flex-col overflow-hidden transition-colors duration-300">
        ...
        <div className="flex-1 overflow-y-auto px-2 py-2 space-y-1">
          {sessions.length === 0 ? (
            <p className="text-xs text-slate-400 dark:text-slate-500 text-center mt-4">No chats yet</p>
          ) : (
              ...
              <div
                ...
                className={`group flex items-center gap-2 px-3 py-2 rounded-xl cursor-pointer transition-all ${
                  activeSessionId === session._id
                    ...
                    : 'hover:bg-orange-50 dark:hover:bg-slate-700 border border-transparent'
                }`}
                ...
                {renamingId === session._id ? (
                  <div className="flex items-center gap-1 flex-1" onClick={(e) => e.stopPropagation()}>
                    <input
                      ...
                      className="flex-1 text-xs bg-white dark:bg-slate-700 border border-orange-200 dark:border-slate-600 rounded-lg px-2 py-1 outline-none focus:ring-1 focus:ring-orange-300 dark:text-slate-200"
                      onKeyDown={(e) => {
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                      ...
                      <button onClick={() => handleDelete(session._id)} className="text-slate-400 hover:text-red-500 transition-colors">
      ...
      <div className="flex-1 flex flex-col bg-white dark:bg-slate-900 rounded-2xl border border-orange-100 dark:border-slate-700 shadow-sm overflow-hidden transition-colors duration-300">
        <div className="flex-1 overflow-y-auto px-4 py-5 bg-gradient-to-b from-orange-50/50 via-white to-white dark:from-slate-900 dark:via-slate-900 dark:to-slate-950">
          {history.length === 0 ? (
            <div className="flex flex-col items-center justify-center text-center min-h-[400px] px-6">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 max-w-sm">Ask me anything about the document!</p>
            </div>
          ) : (
          ...
          {loading && (
            <div className="flex items-start gap-3 mt-4">
              ...
              <div className="bg-orange-50 dark:bg-slate-800 border border-orange-100 dark:border-slate-600 rounded-2xl rounded-tl-md px-4 py-3 shadow-sm">
        ...
        <div className="border-t border-orange-100 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3">
          <form
            onSubmit={handleSendMessage}
            className="flex items-center gap-3 bg-orange-50 dark:bg-slate-800 border border-orange-100 dark:border-slate-600 rounded-2xl px-3 py-2 shadow-sm focus-within:ring-2 focus-within:ring-orange-300 dark:focus-within:ring-orange-500/40 transition-all"
          >
            <input
              type="text"
              value={message}
              ...
              className="flex-1 bg-transparent outline-none text-sm text-slate-700 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-400 px-2 py-2"
              disabled={loading}
            />
            <button
              type="submit"
